...
Evaluation of our research questions and how they help us going into modeling. Then start writing about the modeling process here. 
\subsubsection{Legendary classifier}
Details about this model evaluation and validation metrics go here like cross validation and confusion tables. we can show tables comparing tuned and untuned and we need to say WHY the metrics were selected during tuning and which one is better for final model selection.

\subsubsection{Primary type classifier}
Same as above just for this target. 

\section{Results}
Here we will talk about the results of the two(four) models and show the summary tables. We should have two tables one for the legendary results for RF and LG after tuning and CV, and one for the type results. 

\section{Discussion and Conclusion}
We need to talk about why we got good results for the legendary classified and why the primary type classifier does so bad. We can mention binary classifiers being easier, as well as the primary type not having any strongly correlating features in the dataset so to be able to improve it we need to find richer data which might not exist. State again our three research questions and our answers to each one of them directly, tying it back to the model results. Mention what we learned that was unexpected and interesting and wrap up the paper by saying what we can improve in future works. 
